    ...
    var progdata = data.data;
      ...
      if(typeof(progdata.episode_ids)=='undefined' || progdata.episode_ids.length==0) $('#program_details_episodes_table').replaceWith(htmlspecialchars(OB.t('Program Details','No episodes found')));
        ...
        $.each(progdata.episode_ids, function(index,episode) {
          ...
          {
	   $('#program_details_episodes_table').append('<tr><td>'+htmlspecialchars(episode.title)+'</td><td>'+format_timestamp(episode.updated)+'</td><td>'+secsToTime(episode.duration,"hms")+'</td><td><a href="javascript:OB.Sidebar.playerPlay(\'program\',\'audio\','+episode.id+')">Preview</a></td></tr>');
          }
	//handle credits
